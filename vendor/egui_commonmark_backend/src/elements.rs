...
pub fn code_block<'t>(
    ...
    // NotepadMD+ patch: render as a selectable Label instead of a read-only
    // TextEdit so the code text participates in egui's cross-label text
    // selection — a selection can be dragged across paragraphs *and* code
    // blocks and copied as one.
    let margin = 8.0_f32;
    let response = egui::Frame::new()
        .inner_margin(egui::Margin::same(margin as i8))
        .show(ui, |ui| {
            let buf: &dyn TextBuffer = &text;
            let galley = layouter(ui, buf, max_width - 2.0 * margin);
            ui.add(egui::Label::new(galley));
        })
        .response;

    // Background color + frame, painted full-width like the old TextEdit
    let mut frame_rect = response.rect;
    frame_rect.set_width(frame_rect.width().max(max_width));
    ui.painter().set(
    ...
    let persistent_id = ui.make_persistent_id(response.id);
    ...
    if copy_button.clicked() {
        // the button always copies the whole block; partial copies go through
        // the global label text selection instead
